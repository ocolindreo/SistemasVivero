...
function UsuariosView({ currentUser, onToast, onSessionInvalid }) {
  ...
  return <section className="users-view" aria-labelledby="users-title">
    ...
    {currentUser.rol.codigo !== 'ADMIN' ? <div className="empty-state">No tiene permisos para administrar usuarios.</div> : <div className="data-card"><input className="user-search" placeholder="Buscar por usuario, nombre o correo" value={search} onChange={(event) => setSearch(event.target.value)} />{loading ? <div className="empty-state">Cargando usuarios...</div> : <DataTable columns={columns} data={filtered} getRowKey={(user) => user.id} emptyMessage={users.length ? 'No hay resultados para la búsqueda.' : 'No hay usuarios registrados.'} />}</div>}
