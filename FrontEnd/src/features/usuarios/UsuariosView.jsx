...
function UsuariosView({ currentUser, onToast, onSessionInvalid }) {
  ...
  const canManageUsers = ['ADMIN', 'VIVERO'].includes(currentUser.rol.codigo)
  const rolesPermitidos = useMemo(() => currentUser.rol.codigo === 'ADMIN' ? roles : roles.filter((role) => role.codigo !== 'ADMIN'), [roles, currentUser.rol.codigo])
  ...
  const filtered = useMemo(() => users.filter((user) => currentUser.rol.codigo === 'ADMIN' || user.rol?.codigo !== 'ADMIN').filter((user) => `${user.username} ${user.nombres} ${user.apellidos} ${user.email}`.toLowerCase().includes(search.toLowerCase())), [users, search, currentUser.rol.codigo])
  const columns = [
    { key: 'username', label: 'Usuario', sortable: true },
    { key: 'nombre', label: 'Nombre', sortable: true, sortValue: (user) => `${user.nombres} ${user.apellidos}`, render: (user) => `${user.nombres} ${user.apellidos}` },
  ...
  return <section className="users-view" aria-labelledby="users-title">
    ...
    {!canManageUsers ? <div className="empty-state">No tiene permisos para administrar usuarios.</div> : <div className="data-card"><input className="user-search" placeholder="Buscar por usuario, nombre o correo" value={search} onChange={(event) => setSearch(event.target.value)} />{loading ? <div className="empty-state">Cargando usuarios...</div> : <DataTable columns={columns} data={filtered} getRowKey={(user) => user.id} emptyMessage={users.length ? 'No hay resultados para la búsqueda.' : 'No hay usuarios registrados.'} />}</div>}
    {modal && <Modal title={modal.user ? 'Editar usuario' : 'Nuevo usuario'} onClose={() => setModal(null)}><UsuarioForm usuario={modal.user} roles={rolesPermitidos} saving={saving} onSave={save} onClose={() => setModal(null)} currentUserId={currentUser.id} /></Modal>}
