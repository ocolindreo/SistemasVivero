...
import ConstanciaEntrega from './ConstanciaEntrega'
...
const READ_ROLES = ['ADMIN', 'VIVERO', 'GESTION', 'CONSULTA']
...
function EntregaDetalle({ entrega, impresion, loading, canWrite, canPrint, onBack, onAction, onConfirm, onPrint }) {
...
function EntregasView({ currentUser, onToast, onSessionInvalid }) {
  ...
  if (detalle) return <><EntregaDetalle entrega={detalle} impresion={impresion} loading={loadingDetalle} canWrite={canWrite && !saving} canPrint={canPrint} onBack={() => setDetalle(null)} onAction={ejecutarAccion} onConfirm={abrirConfirmacion} onPrint={() => { flushSync(() => setImpresion({ fecha: new Date(), username: currentUser?.username || '—' })); window.print() }} />{modal?.kind === 'confirmar' && <ConfirmarEntregaModal entrega={detalle} modal={modal} saving={saving} onChange={updateConfirmField} onDetailChange={updateConfirmDetail} onSubmit={guardarConfirmacion} onClose={() => setModal(null)} />}</>
